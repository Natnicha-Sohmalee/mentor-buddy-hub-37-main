...
function Home() {
  ...
  useEffect(() => { Promise.all([selectRows<Assignment>("task_assignments", "status"), selectRows<{ id: string }>("projects", "id")]).then(([tasks, list]) => { setAssignments(tasks); setProjects(list); }).catch((e) => setError(e instanceof Error ? e.message : "โหลดข้อมูลไม่สำเร็จ")); }, []);
