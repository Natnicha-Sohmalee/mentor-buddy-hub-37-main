...
type Project = { id: string; name: string; type: string | null; owner_type: string; created_at: string };
...
function ProjectList() {
  ...
  useEffect(() => { selectRows<Project>("projects", "id,name,type,owner_type,created_at", "", "created_at.desc").then(setProjects).catch((e) => setError(e instanceof Error ? e.message : "โหลดโปรเจ็คไม่สำเร็จ")); }, []);
  return <AppShell title="โปรเจ็ค" description="รายการโปรเจ็คที่บัญชีของคุณมีสิทธิ์เข้าถึง" actions={role === "admin" ? <Button asChild><Link to="/projects/create">สร้างโปรเจ็ค</Link></Button> : undefined}>
    ...
    {!error && <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{projects.map((project) => <article key={project.id} className="surface p-5"><div className="flex items-start justify-between gap-2"><h2 className="font-semibold">{project.name}</h2>{project.type && <Badge variant="secondary">{project.type}</Badge>}</div><p className="mt-3 text-sm text-muted-foreground">เจ้าของ: {project.owner_type === "mentor" ? "พี่เลี้ยง" : "บ้าน"}</p><p className="mt-1 text-xs text-muted-foreground">สร้างเมื่อ {new Date(project.created_at).toLocaleDateString("th-TH")}</p></article>)}</div>}
